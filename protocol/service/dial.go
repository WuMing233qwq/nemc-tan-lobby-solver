...
func (d *Dialer) enterTanLobbyRoom(ctx context.Context, tanLobbyLoginResp bunker.TanLobbyLoginResponse) (
	remoteNetherNetID string,
	err error,
) {
	// Generate client nether ID and parse basic info
	d.clientNetherID = fmt.Sprintf("%d", rand.Uint64())
	...
	if !ok {
		return "", fmt.Errorf("enterTanLobbyRoom: Expect the incoming packet is *packet.TanLoginResponse, but got %#v", pk)
	}
	if tanLoginResp.ErrorCode != packet.TanLoginSuccess {
		return "", fmt.Errorf("enterTanLobbyRoom: Login failed (code = %d)", tanLoginResp.ErrorCode)
	}
	...
	if !ok {
		return "", fmt.Errorf("enterTanLobbyRoom: Expect the incoming packet is *packet.TanEnterRoomResponse, but got %#v", pk)
	}
	if tanEnterRoomResp.ErrorCode != packet.TanEnterRoomSuccess {
		...
		case packet.TanEnterRoomNotFound:
			return "", fmt.Errorf("enterTanLobbyRoom: Target room (%d) is closed", roomID)
		case packet.TanEnterRoomAccessDenied:
			return "", fmt.Errorf("enterTanLobbyRoom: Access denied (Possibly is that you are not the friend of the owner)")
		case packet.TanEnterRoomFullOfPeople:
			return "", fmt.Errorf("enterTanLobbyRoom: Target room (%d) is full of people", roomID)
		case packet.TanEnterRoomWrongPasscode:
			return "", fmt.Errorf("enterTanLobbyRoom: Provided room passcode is incorrect")
		default:
			return "", fmt.Errorf("enterTanLobbyRoom: Enter room failed due to unknown reason (code = %d)", tanEnterRoomResp.ErrorCode)
		}
	}
	...
	for {
		...
		case *packet.TanNotifyServerReady:
			return p.NetherNetID, nil
		case *packet.TanKickOutResponse:
			return "", fmt.Errorf("enterTanLobbyRoom: The host owner kick you from the room")
		case *packet.TanNewGuestResponse, *packet.TanSetTagListResponse:
		default:
			return "", fmt.Errorf("enterTanLobbyRoom: Unknown packet received; pk = %#v", pk)
		}
	}
}
