...
// ParseNetEaseClientData extracts the ClientData (skin, third-party name, etc.) from a NetEase login
// request. NetEase clients send a non-standard/empty certificate chain that the regular Parse rejects
// (e.g. "decode chain: no elements") before it ever reads the client-data token, so the standard path
// yields neither skin nor username. This function skips the chain and reads the client-data JWT
// directly, parsing its claims WITHOUT signature verification (the token is self-signed with a key
// NetEase does not expose through a chain). It is meant to recover the real player's data so a proxy
// can forward it to a backend server.
func ParseNetEaseClientData(requestData []byte) (ClientData, error) {
	var cData ClientData
	buf := bytes.NewBuffer(requestData)

	var chainLength int32
	if err := binary.Read(buf, binary.LittleEndian, &chainLength); err != nil {
		return cData, fmt.Errorf("read chain length: %w", err)
	}
	if chainLength < 0 || int(chainLength) > buf.Len() {
		return cData, fmt.Errorf("invalid chain length: %d", chainLength)
	}
	buf.Next(int(chainLength)) // Skip the (non-standard) chain; we only want the client-data token.

	var rawLength int32
	if err := binary.Read(buf, binary.LittleEndian, &rawLength); err != nil {
		return cData, fmt.Errorf("read client data token length: %w", err)
	}
	if rawLength < 0 || int(rawLength) > buf.Len() {
		return cData, fmt.Errorf("invalid client data token length: %d", rawLength)
	}
	rawToken := string(buf.Next(int(rawLength)))

	tok, err := jwt.ParseSigned(rawToken, []jose.SignatureAlgorithm{jose.ES384})
	if err != nil {
		return cData, fmt.Errorf("parse client data token: %w", err)
	}
	if err := tok.UnsafeClaimsWithoutVerification(&cData); err != nil {
		return cData, fmt.Errorf("read client data claims: %w", err)
	}
	return cData, nil
}
